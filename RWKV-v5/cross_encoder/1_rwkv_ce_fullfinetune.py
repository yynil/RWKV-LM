...
if __name__ == '__main__':
    ...
    parser.add_argument('--tuner', type=str, default='lora', help='tuner type', choices=['ft', 'lora'])
    ...
    from src.model_for_classification import RwkvForClassification
    ...
    def collate_fn(examples):
        idx = torch.tensor([x['idx'] for x in examples], dtype=torch.long)
        targets = torch.tensor([x['targets'] for x in examples], dtype=torch.long)
        return idx, targets
    ...
    args.wandb = 'rwkv5_cross_encoder_fullfinetune'
    args.run_name = 'yy' 
    args.my_qa_mask = 0
    args.num_nodes = 1
    args.devices = 1
