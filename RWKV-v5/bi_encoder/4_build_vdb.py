...
def remove_invisible_and_illegal_utf8(s):
    # Remove invisible characters
    s = regex.sub(r'\p{C}+', '', s)

    # Remove illegal UTF-8 characters
    ...
    for file in files:
        if file.endswith('.txt'):
            ...
            for line in text_lines:
                line = remove_invisible_and_illegal_utf8(line)
                grouped_text += line + '\n'
                if len(grouped_text) >= chunk_size:
                    texts[file_name_without_ext+'_'+str(index)] = f"《{file_name_without_ext}》："+grouped_text
                    index += 1
                    grouped_text = ""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--text_dir', type=str, default='/home/yueyulin/下载/novels')
    parser.add_argument('--vdb_dir',type=str, default='/home/yueyulin/下载/novels_vdb')
    parser.add_argument('--chunk_size',type=int, default=1000)
    args = parser.parse_args()

    os.makedirs(args.vdb_dir,exist_ok=True)
    texts = read_texts_dir(args.text_dir,args.chunk_size)
    print(texts.keys())
    ...
    progress_bar = tqdm(texts.keys(),desc='inference')
    for key in progress_bar:
        embedding = inference(model, tokenizer, texts[key]).tolist()
        documents.append(texts[key])
        embeddings.append(embedding)
        novel_name,chapter = key.split('_')
        metadatas.append({"novel_name":novel_name,"chapter":chapter})
        ids.append(key)
        if len(ids) >= 1000:
            collection.add(
                documents=documents,
