...
if __name__ == '__main__':
    ...
    train_data = load_from_disk(data_file)
    print(train_data[0])
    print(train_data[1])

    def collate_fn(examples):
        idx = torch.tensor([x['input_ids'] for x in examples], dtype=torch.long)
        targets = torch.tensor([x['labels'] for x in examples], dtype=torch.long)
        return idx, targets
    data_loader = DataLoader(train_data, shuffle=True, pin_memory=True, batch_size=cmd_args.bs, num_workers=24, persistent_workers=True, drop_last=True, collate_fn=collate_fn)
    ...
    if '1B5' in args.model_file:
        args.wandb = 'rwkv5_sft_att_ffn_1b5'
    elif '3B' in args.model_file:
        args.wandb = 'rwkv5_sft_att_ffn_3b'
    elif '0.4B' in args.model_file:
        args.wandb = 'rwkv5_sft_att_ffn_0.4b'
    elif '7b' in args.model_file or '7B' in args.model_file:
        args.wandb = 'rwkv5_sft_att_ffn_7b'
    else:
        args.wandb = 'rwkv5_sft_att_ffn_'+args.model_file.split('/')[-1]
    args.run_name = 'yy' 
    args.my_qa_mask = 0
    args.num_nodes = 1
    args.devices = 1
