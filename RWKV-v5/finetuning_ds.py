...
def load_json_as_ds(tokenizer, input_json_file, ctx_len):
    ...
    def process_example(example):
        ...
        nonlocal max_length, count, min_length,total_length
        max_length = max(max_length, length_of_input_ids)
        min_length = min(min_length, length_of_input_ids)
        count += 1
        total_ength += length_of_input_ids
    ...
    print(f"Statistics of length of input_ids:")
    print(f"Max length: {max_length}")
    print(f"Min length: {min_length}")
    print(f"Total count: {count}")
    print(f"Average length: {total_length / count}")
    return ds
