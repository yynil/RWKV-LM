...
"""
    ...
    if args.data_type == 'wds_img':
        ...
        from src.model import BiRWKV
        model = BiRWKV(args)
        print(args)
        if args.load_model != "":
            print(f'load from {args.load_model}')
            model.load_from_path(args.load_model)
        else:
    ...
    print(input_ids)
    print(tokenizer.decode([10902, 10362]))
    input_ids[6:8]=[mask_token_id,mask_token_id]
    print(input_ids)
